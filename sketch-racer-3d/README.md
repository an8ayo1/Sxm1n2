...
- 하늘색 하늘, 파스텔 건물, 한국식 상가와 아파트, 가로수·횡단보도·신호등을 배치했습니다.
...
- 한글 간판은 설치된 macOS/Windows/Noto 한국어 폰트를 사용합니다. 해당 폰트가 없으면 영문 간판으로
  표시하므로 별도 폰트 다운로드 없이 실행됩니다. 신호등은 폐쇄된 레이스 코스의 녹색 고정 장식입니다.
...
- 도로를 23m에서 **32m**로 넓혔습니다. 연석·보도·건물·게이트·신호등도 폭에 맞춰 이동합니다.
...
- **아트:** 원본 스케치의 각진 적재함·기울어진 지붕·큰 바퀴를 코드로 재해석했습니다.
  외부 차량 모델, 유료 서비스, 이미지 생성 API를 사용하지 않습니다.
